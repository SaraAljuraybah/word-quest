...
export function VirtualKeyboard({ colors, onLetter, onBackspace, onSubmit, disabled }) {
  return (
    <div className="mx-auto w-full max-w-3xl space-y-1.5 rounded-3xl border border-slate-200/70 bg-white/80 p-2 shadow-premium backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.05] sm:space-y-2 sm:p-3">
      {ARABIC_KEYBOARD_ROWS.map((row, rowIndex) => (
        <div key={row.join("")} className="grid gap-1.5 sm:gap-2" style={{ gridTemplateColumns: `repeat(${row.length}, minmax(0, 1fr))` }}>
          {row.map((letter) => {
